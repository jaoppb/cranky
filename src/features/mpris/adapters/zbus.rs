...
impl ZbusMprisAdapter {
    ...
    fn update_state_from_props(
        state: &mut PlayerState,
        props: &crate::shared::dbus::domain::PropertiesMap,
    ) {
                ...
                if name.starts_with("org.mpris.MediaPlayer2.") {
                    ...
                    if event.is_new() {
                            ...
                            .await
                        {
                            ...
                            if let Ok(mut prop_stream) = conn
                                .subscribe_properties_changed(BusType::Session, &dest, &path)
                                .await
                            {
                                ...
                                let dest_clone = dest.clone();
                                tokio::spawn(async move {
                                    while let Some((changed_iface, changed_props)) =
                                        prop_stream.next().await
                                    {
                                        if changed_iface.as_str() == "org.mpris.MediaPlayer2.Player"
                                        {
                                            ...
                                            if let Some(mut player) =
                                                mpris_state.players.get(p_name).cloned()
                                            {
                                                Self::update_state_from_props(
                                                    &mut player,
                                                    &changed_props,
                                                );
                                                mpris_state
                                                    .players
                                                    .insert(p_name.to_string(), player);
