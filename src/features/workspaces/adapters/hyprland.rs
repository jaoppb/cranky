use crate::features::workspaces::adapters::hyprland_provider::{HyprlandProvider, RealHyprlandProvider};
...
impl HyprWorkspaceDto {
    fn into_domain(self) -> Workspace {
        Workspace::new(
            ...
            Some(crate::features::workspaces::domain::MonitorName::new(self.monitor)),
        )
    }
}
...
impl HyprMonitorDto {
    fn into_domain(self) -> (Monitor, bool) {
        let special_ws_id = if self.special_workspace.id != 0 {
            Some(crate::features::workspaces::domain::WorkspaceId::new(self.special_workspace.id))
        } else {
            None
        };
...
impl HyprlandAdapter {
    ...
    fn parse_event(raw_line: &str) -> Option<crate::shared::events::core::WindowManagerEvent> {
        use crate::shared::events::core::{WindowManagerEvent, WindowAddress, WindowTitle};
...
impl WindowManagerPort for HyprlandAdapter {
    ...
    fn get_state(&self) -> Result<crate::features::workspaces::ports::WindowManagerState, WindowManagerError> {
        let ws_json =
            self.provider
                .query_workspaces()
...
mod tests {
    ...
    async fn test_hyprland_adapter_get_state_valid() {
        ...
        assert_eq!(res.2, Some(crate::features::workspaces::domain::MonitorName::new("DP-1"))); // focused
    }
