...
use crate::shared::dbus::domain::{BusType, DBusState, DBusSubscription, DBusValue};
use crate::shared::events::signals::SignalHub;
use crate::shared::dbus::ports::{DBusPort, DBusPortError};
...
mod tests {
    ...
    fn test_parse_value_primitives() {
        assert_eq!(
            ZbusAdapter::parse_value(&Value::Str("hello".into())),
            DBusValue::String("hello".into())
        );
        assert_eq!(
            ZbusAdapter::parse_value(&Value::U8(255)),
            DBusValue::Int(255)
        );
        assert_eq!(
            ZbusAdapter::parse_value(&Value::I16(-100)),
            DBusValue::Int(-100)
        );
        assert_eq!(
            ZbusAdapter::parse_value(&Value::U16(100)),
            DBusValue::Int(100)
        );
        assert_eq!(
            ZbusAdapter::parse_value(&Value::I32(-1000)),
            DBusValue::Int(-1000)
        );
        assert_eq!(
            ZbusAdapter::parse_value(&Value::U32(1000)),
            DBusValue::Int(1000)
        );
        assert_eq!(
            ZbusAdapter::parse_value(&Value::I64(-10000)),
            DBusValue::Int(-10000)
        );
        assert_eq!(
            ZbusAdapter::parse_value(&Value::U64(10000)),
            DBusValue::Int(10000)
        );
        assert_eq!(
            ZbusAdapter::parse_value(&Value::F64(3.5)),
            DBusValue::Float(3.5)
        );
        assert_eq!(
            ZbusAdapter::parse_value(&Value::Bool(true)),
            DBusValue::Bool(true)
        );
        assert_eq!(
            ZbusAdapter::parse_value(&Value::Bool(false)),
    ...
    fn test_parse_value_array() {
        ...
        assert_eq!(
            parsed,
            DBusValue::Array(vec![DBusValue::Int(1), DBusValue::Int(2)])
        );
    }
    ...
    fn test_parse_value_dict() {
        ...
        dict.append(Value::Str("key1".into()), Value::I32(42))
            .unwrap();
